...
module tb_clock_divider;
    ...
    initial begin
        step;
        check({divided,tick,div2,tick2}===4'b0000,"reset");
        ...
        rst=1;
        #1;
        check(tick===0,"reset masks tick before edge");

        step;
        check(divided===0,"reset mid period");

        rst=0;

        repeat(4) begin
            step;
            check(divided===0,"restart low half");
        end

        step;
        check(divided===1,"restart first transition");
